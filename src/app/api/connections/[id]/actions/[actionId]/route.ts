...
export async function GET(_req: NextRequest, ctx: Params) {
  ...
  if (roleAtLeast(access.role, "editor")) return NextResponse.json(action);
...
export async function PUT(req: NextRequest, ctx: Params) {
  ...
  try {
    const action = await updateAction(session.user.id, id, actionId, parsed.data);
    if (!action) return NextResponse.json({ category: "not_found" }, { status: 404 });
    ...
    return NextResponse.json(
      { category: "server", message: (e as Error).message ?? "Failed to update action." },
      { status: 500 },
    );
  }
}
