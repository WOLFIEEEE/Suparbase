...
import type { Schema, Table } from "@/lib/schema/types";
...
export function FkBadge({ table, columnName, value, allValuesOnPage, schema }: FkBadgeProps) {
  ...
  const { data: labels } = useReferenceLabels(table, columnName, valuesKey, schema);
  ...
  return (
    <Link
      to={`/tables/${encodeURIComponent(fk.table)}/${encodeURIComponent(String(value))}`}
