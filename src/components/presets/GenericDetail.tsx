...
import { ArrowUpRight, KeyRound, Pencil, Sparkles, Trash2 } from "lucide-react";
...
const META_RE = /^(created_at|updated_at|inserted_at|deleted_at)$/i;
...
/**
 * Detail page for generic-classified tables. Mirrors UserDetail in shape:
 * - PageHeader with breadcrumbs.
 * - Hero card surfaces the label column (analysis.primary.titleColumn or PK).
 * - Identity sections respect analysis.hiddenColumns.
...
export function GenericDetail({ connectionId, table, schema, analysis, pkSegment }: Props) {
  ...
  const heroCols = useMemo(
    () => new Set([titleCol, subtitleCol, statusCol].filter(Boolean) as string[]),
    [titleCol, subtitleCol, statusCol],
  );
  ...
  if (error) {
    return (
      <div className="space-y-6">
        <PageHeader breadcrumbs={breadcrumbs} title="Row" />
        <ErrorBanner
  ...
  const fallbackId =
    table.primaryKey.length > 0
      ...
      : "row";
  const title =
    titleCol && row[titleCol] != null ? String(row[titleCol]) : fallbackId;
  const subtitle =
    subtitleCol && row[subtitleCol] != null ? String(row[subtitleCol]) : null;
  const status =
    statusCol && row[statusCol] != null ? row[statusCol] : null;
  const display = title || fallbackId || "row";
  ...
  async function performDelete() {
    ...
    try {
      ...
      toast.success(`Deleted row from ${table.name}`, {
        duration: 5000,
        action: {
          label: "Undo",
          onClick: async () => {
          ...
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div className="space-y-1">
              ...
              <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-fg-muted">
                ...
                {table.kind === "view" && <Badge tone="warn">read-only view</Badge>}
              </div>
            </div>
            {status != null && (
              <StatusPill
                value={typeof status === "boolean" ? (status ? "active" : "inactive") : String(status)}
              />
            )}
          </div>
          {table.primaryKey.length > 0 && (
            <div className="flex items-center gap-1.5 border-t hairline pt-3 text-[11px] text-fg-faint">
              <KeyRound className="h-3 w-3 text-accent" aria-hidden />
              <span>
                {table.primaryKey.map((c) => (
                  <span key={c}>
                    <code className="font-mono text-fg-muted">{c}</code>={" "}
                    <code className="font-mono text-fg">{String(row[c] ?? "")}</code>
                    {c !== table.primaryKey[table.primaryKey.length - 1] && ", "}
                  </span>
                ))}
              </span>
            </div>
          ...
          <aside className="space-y-4">
            <section className="surface rounded-md p-5">
              ...
              {incomingRefs.length === 0 ? (
                <p className="text-xs text-fg-muted">No other tables reference this row.</p>
              ) : (
